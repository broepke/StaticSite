Title: A Comprehensive Guide to Binary Classification
Subtitle: How to Properly Build a Machine Learning Classification Model on Text to Classify Product Reviews
Date: 2022-01-22
Modified: 2022-01-22
Status: draft
...
We can see that the string is perfectly clean, free of *numbers*, *punctions*, *stop words*, and the words are *stemmed* into their most simple forms. We're *almost* ready to start building our model! 
...
That is good enough to let us know that **the data is imbalanced**. There is three times the number of positive class `1` observations versus the negative class `0`. We will have to make sure that we handle the imbalance appropriately in our model. I'll cover a couple of different ways in this article. 
...
Next we have a function for creating a [Pipeline](({filename}sklearnpipelines.md)). A Pipeline allows us to consistently preprocess our data and connect various additional steps in a sequential workflow. Let's break down what's happening below.
...
Now it's time to **Cross Validate** two classifiers. Cross-validation is the process of partitioning data into *n* number of different splits, which you then use to validate your model. Cross-Validation is so important because, in some cases, the observations it learns from in the train set might not represent the observations in the test set. Therefore, you avoid this by running through different data slices. Read more about this on [Scikit-Learn's documentation](https://scikit-learn.org/stable/modules/cross_validation.html).
...
Finally, we will use a classifier that supports the `class_weight` parameter regarding how we're dealing with imbalanced data. A handful of models from Scikit-Learn support this, and simply by setting the value to `balanced`, we can account for imbalanced data. Other methods include [SMOTE](https://www.jair.org/index.php/jair/article/view/10302), but this is a great place to start. You can read more about [working with imbalanced data](({filename}imbalanced.md)) from my other post.
...
There you have it! An end-to-end example of building a binary classifier. It can be simple to build one in a few steps, but this post outlines the preferred steps you should use to *properly* build your classifier. We started with cleaning the data and some lightweight *Feature Engineering* followed by *Model Selection*, *Model Training*, and *Evaluation*, and finally, *Persisting the Model* and then testing it on new data. This workflow should work for almost any binary or multi-class classification problem. Enjoy and happy model building!

*There are two other steps I left out of here which I'll cover at a later time.  **Feature Selection** and **Hyperparameter Tuning**.  Depending on the complexity of your data you will want to dig into Feature Selection and to squeeze out the most performace of your model check out Hyperparmeter Tuning.*
